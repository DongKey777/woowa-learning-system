...
```
corpus/
├── concepts/                      # 3319 JSON entities (1 concept = 1 file)
│   ├── algorithm/*.json
│   ├── data-structure/*.json
│   ├── database/*.json
│   ├── design-pattern/*.json
...
│   ├── spring/*.json
│   └── system-design/*.json
├── concept_graph.json             # ~5MB — nodes (3319) + edges (prerequisite 6122, confusable_with N)
└── schemas/
...
```json
{
  ...
  "edges": {
    "prerequisite": [["from_id", "to_id"], ...],   # 6122 pairs
    "confusable_with": [...],
    "extends": [...]
  },
  "stats": {...}
