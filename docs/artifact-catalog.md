...
```
corpus/
├── concepts/                      # 3239 JSON entities (1 concept = 1 file)
│   ├── algorithm/*.json
│   ├── data-structure/*.json
│   ├── database/*.json
│   ├── design-pattern/*.json
...
│   ├── spring/*.json
│   └── system-design/*.json
├── concept_graph.json             # ~5MB — nodes (3239) + edges (prerequisite 5882, confusable_with N)
└── schemas/
...
```json
{
  ...
  "edges": {
    "prerequisite": [["from_id", "to_id"], ...],   # 5882 pairs
    "confusable_with": [...],
    "extends": [...]
  },
  "stats": {...}
