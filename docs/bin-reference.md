...
```bash
bin/index-fetch [--tag paradigm-v2-index-v1.0.6] [--force] [--auto-upgrade] [--expected-sha256 <hex>]
```
...
cp state/index.tar.zst /tmp/paradigm-v2-index-v1.0.6
gh release create paradigm-v2-index-v1.0.6 /tmp/paradigm-v2-index-v1.0.6 \
    --title "woowa-learning-system Lance index v1.0.5" --notes "...SHA256..."
# 이후 학습자는 bin/index-fetch --tag paradigm-v2-index-v1.0.6 로 업데이트
```
...
- 결과: `corpus/concept_graph.json` (~5MB, 3654 nodes / 6931 prereq edges)
...
위 51은 Phase T-X wrapper만 센 것이다. 여기에 상단 core entries + 위 "Mode feature builders" 15개 + Live PR review cycle의 `bin/pr-thread-status`(신규) + corpus 품질 게이트 `bin/corpus-lint`(exact-shortcut 소유권 회귀 차단)·`bin/corpus-relations-lint`(confusable_with 대칭/dangling/self 무결성, index-free)·`bin/corpus-gate`(v4 A-F 게이트 한 wrapper)까지 더한 **Total bin/\* in woowa-learning-system: 87 entries** (`find bin -maxdepth 1 -type f`). 학습자 외울 명령 = 0개.
