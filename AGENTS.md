...
| 매 coach turn 답변 직후 | 기본 `bin/learn-response-quality --source-event-id <id> --summary-only --contract-flag body_not_captured --contract-flag token_efficient_summary_only --silent`; zero-copy 파일 capture 가능 시 `--response-path <answer.md>`로 최종 답변 전체 저장 |
