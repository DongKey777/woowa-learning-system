...
├── bin/        # 85 user/operator commands; thin wrappers over core/rag/mission
...
bin/index-fetch
```
GitHub Releases `DongKey777/woowa-learning-system` → `paradigm-v2-index-v1.0.6` (release별 약 13-19MB, SHA256 검증) → `state/index/`. 소요 ~15초.
...
위 Phase T-X 외에 **Mode feature builders 15개**(`anchors-build`, `learn-evidence-sync`, `pr-diff-evolution-build`, `learn-pr-meta-build`, `learn-pr-review-build`, `learn-predict-build`, `reviewer-profile-build`, `learn-temporal-build`, `learn-thread-recon-build`, `learn-cohort-build`, `peer-pr-build`, `learn-cross-mission-build`, `learn-learning-path-build`, `learn-memory-review-build`, `learn-meta-analytics-build` — §4.2.2 고급 모드 artifact 사전 빌드)와 Live PR review cycle `bin/pr-thread-status`(라이브 정합), corpus 품질 게이트 `bin/corpus-lint`(exact-shortcut 소유권)·`bin/corpus-relations-lint`(confusable_with 무결성)·`bin/corpus-gate`(v4 A-F 게이트 wrapper)가 더 있다. woowa-learning-system `bin/*` 합계: **87 entries**. 학습자 외울 명령 = 0개, AI 세션이 의도 감지로 자동 호출. 자세한 usage는 [`docs/bin-reference.md`](docs/bin-reference.md).
