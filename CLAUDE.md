...
| 매 coach turn 답변 직후 (필수) | 기본 `bin/learn-response-quality --source-event-id <id> --response-file - --silent`; zero-copy 파일 capture 가능 시 `--response-path <answer.md>`로 최종 답변 전체 저장 |
...
- 회귀 검증: `pytest tests/ -q` 모든 변경 후. 현재 508 passed 유지.
